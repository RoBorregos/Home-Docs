# Overview

Welcome to the RoBorregos @Home publications section. Here you can find academic papers, conference proceedings, and research work produced by our team members related to robotics, service robots, and RoboCup@Home competition.

## Research Areas

Our research focuses on several key areas in service robotics:

- **Task Planning and Natural Language Processing**: Using LLMs and parsing techniques for reliable robot task planning
- **Digital Twins**: Implementation of digital twins for mobile and collaborative robot scenarios
- **Robotic Education**: Technology to support discrete event control teaching and robotics education
- **Human-Robot Interaction**: Improving communication and interaction between humans and service robots
- **Computer Vision**: Object detection, recognition, and scene understanding for domestic environments

## Publication Categories

### [Recent Publications](Frida.md)

Explore our latest research contributions, including work on LLM-based task planning for robotics and cutting-edge developments in service robot technology.

### [Team Publications](Team.md)

A comprehensive collection of publications by RoBorregos team members covering various aspects of robotics, engineering education, and digital twin implementations.

---


## Frida Papers

Adán Flores-Ramírez et al. Taming the llm: Reliable task planning for robotics
using parsing and grounding. In Lourdes Martínez-Villaseñor, Roberto A. Vázquez,
and Gilberto Ochoa-Ruiz, editors, Advances in Soft Computing, pages 296–312,
Cham, 2026. Springer Nature Switzerland.
del equipo:


## Team Paper

E Altamirano-Avila, A. Valdivia, and C. Vazquez-Hurtado. [AVV22] Work in
´ Progress: Implementation of a Digital Twin as Technology to support Discrete
Event Control Teaching. 2022 IEEE World Engineering Education Conference
(EDUNINE), DOI: 10.1109/EDUNINE53672.2022. 9782154., 2022.

Altamirano-Avila E. A Digital Twin implementation for Mobile and collabo-
rative robot scenarios for teaching robotics based on Robot Operating System.
2022 IEEE Global Engineering Education Conference (EDUCON), DOI: 10 .
1109/EDUCON52537.2022.9766583., 2022.

---

## Research Impact

Our research contributes to advancing the state-of-the-art in service robotics, particularly in the context of the RoboCup@Home competition. We focus on practical implementations that can be deployed on affordable hardware platforms while maintaining robust performance in real-world domestic environments.

## Collaborations

We collaborate with academic institutions and research groups worldwide to advance the field of service robotics. Our work has been presented at various international conferences and published in peer-reviewed journals.

---